# Car Insurance Lead Bot - Gustavo Melo - FIXED
...
def upload():
    ...
    if step == 5:
        ...
        if success: return jsonify({"message": "Documento do veiculo recebido! Uma foto da CNH tambem seria possivel? Envie usando o botao de anexo 📎."})
        ...
        if success: return jsonify({"message": "CNH recebida! Obrigado por todos estes detalhes, gostaria de acrescentar algo mais? Digite sua resposta."})
        return jsonify({"message": "Erro ao enviar."})
    return jsonify({"message": "Por favor, siga as instrucoes acima."})
